...
function collectTransferArrowColors(st, appData, stationIdx = -1) {
  const colors = [];
  const themeColor = (appData && appData.meta)
    ? getThemeColorByStationIndex(appData.meta, stationIdx, '#00b894')
    : '#00b894';
  colors.push(themeColor);
  if (st && Array.isArray(st.xfer)) {
    st.xfer.forEach((x) => {
      if (!x || !x.color) return;
      colors.push(x.color);
    });
...
function buildTransferDotArrowHTML(st, appData, stationIdx = -1) {
  const colors = collectTransferArrowColors(st, appData, stationIdx);
  if (colors.length <= 1) return '';
  const count = colors.length;
  const step = 360 / count;
  // 加大段间空隙，确保能明显看出“箭头与箭头之间有间隔”
  const gapDeg = Math.max(12, step * 0.24);
  // 预留一部分角度专门给箭头头部，避免看起来像纯圆弧
  ...
  const arcs = colors.map((color, idx) => {
    const startDeg = (-90) + (idx * step) + (gapDeg / 2);
    ...
    const safeColor = escapeHtmlAttr(color);
    ...
    const arcPath = `M ${start.x.toFixed(2)} ${start.y.toFixed(2)} A ${r} ${r} 0 ${largeArcFlag} 1 ${bodyEnd.x.toFixed(2)} ${bodyEnd.y.toFixed(2)}`;
    const headPath = `M ${tipX.toFixed(2)} ${tipY.toFixed(2)} L ${leftX.toFixed(2)} ${leftY.toFixed(2)} L ${rightX.toFixed(2)} ${rightY.toFixed(2)} Z`;
    return `<g class="dot-transfer-segment"><path d="${arcPath}" stroke="${safeColor}" stroke-width="9" stroke-linecap="butt" fill="none"></path><path d="${headPath}" fill="${safeColor}"></path></g>`;
  }).join('');

  return `<div class="dot-transfer-ring" aria-hidden="true"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" class="dot-transfer-ring-svg">${arcs}</svg></div>`;
}

function getThemeColorByStationIndex(meta, stationIdx, fallback = '#00b894') {
  try {
    const idx = Number(stationIdx);
    if (meta && Array.isArray(meta.customColorRanges)) {
      for (const range of meta.customColorRanges) {
        const s = range && range.startIdx !== undefined ? parseInt(range.startIdx, 10) : -1;
        const e = range && range.endIdx !== undefined ? parseInt(range.endIdx, 10) : -1;
        const c = range && range.color ? range.color : null;
  ...
  return (meta && meta.themeColor) || fallback;
}
...
#display-app .h-next .val #d-next-st {
    display: block;
    width: 100%;
    max-width: 100%;
    min-width: 0;
...
/* 环线内侧换乘标签：强制横排（与 C 型一致），子项为 span 直接横排 */
#display-app .node .x-box {
...
#display-app .node .x-box .x-tag .sub {
    writing-mode: horizontal-tb !important;
    direction: ltr !important;
}
#display-app .map-l {
...
#display-app .node .dot .dot-transfer-ring,
#display-app .l-node .dot .dot-transfer-ring {
  position: absolute;
  inset: 0;
  border-radius: 50%;
  overflow: hidden;
...
#display-app .node .dot .dot-transfer-ring-svg,
#display-app .l-node .dot .dot-transfer-ring-svg {
  width: 100%;
  height: 100%;
...
#display-app .node .dot .dot-transfer-segment,
#display-app .l-node .dot .dot-transfer-segment {
  position: absolute;
  opacity: 0.95;
}
...
/* 到达站 7 个站：上方标签（换乘等）稍大 */
#display-app #arrival-screen .l-node .info-top .x-tag,
#display-app #arrival-screen .l-node .info-top .defer {
    font-size: 14px;
    padding: 3px 8px;
    border-radius: 4px;
}
#display-app #arrival-screen .l-node .info-top .x-tag.suspended .sub {
    font-size: 11px;
    padding: 0 3px;
}
/* 到达站 7 个站：站名样式与 C 型一致 */
...
#display-app #arrival-screen .l-node.passed .info-btm .name {
    color: #999;
    opacity: 0.7;
}
#display-app #arrival-screen .l-node.passed .info-btm .en {
    color: #ccc;
    opacity: 0.7;
}
/* 到达站 7 站英文换行内层：与 C 型一致 */
#display-app #arrival-screen .l-node .info-btm .c-type-en-inner {
    display: table !important;
    table-layout: fixed !important;
    width: 220px !important;
    max-width: 220px !important;
...
function getNextValidSt(currentIdx, step, appData) {
  ...
  const hasShortTurn = (meta.startIdx !== undefined && meta.startIdx !== -1) || (meta.termIdx !== undefined && meta.termIdx !== -1);
  for (let i = 0; i < len; i++) {
    nextIdx += dir;
    if (meta.mode === 'loop') {
      // 环线 + 短交路：与直线一致，不绕环，限制在运营区内，避免“上一站/下一站”卡出运营区域
      if (hasShortTurn) {
...
function mkNode(st, i, mode, appData, rtState) {
  ...
  if (mode === 'loop') {
    ...
    if (st.xfer && st.xfer.length > 0) {
      ...
      // 与 C 型一致：标签直接平铺，不包一层 div，便于 x-box 横排
      if (xferTags.length > 0) {
        xferHTMLFormatted = xferTags.join('');
      }
    }
    // 默认顺序：名字在前，换乘标签在后（在drawRing中会根据上下半环调整）
    const dotArrowHTML = buildTransferDotArrowHTML(st, appData, i);
    node.innerHTML = `<div class="dot"${dotStyle}>${dotArrowHTML}</div><div class="n-txt"><div style="${nameStyle}">${parsedName}</div><div class="en">${parsedEn}</div>${deferTag}${dockTag}${expressStopTag}<div class="x-box">${xferHTMLFormatted}</div></div>`;
  } else {
    // 线性模式：标签每个单独一行（一栏）
    ...
    node.innerHTML = `
      ...
      <div class="dot"${dotStyle}>${dotArrowHTML}</div>
      <div class="info-btm">
...
export function initDisplayWindow(rootElement) {
  ...
  try {
    ...
    const layoutParam = urlParams && urlParams.get('layout');
    if (layoutParam === 'c-type' || layoutParam === 'ctype') {
      layoutMode = 'c-type';
    } else if (layoutParam === 'linear') {
      layoutMode = 'linear';
    } else {
      // 按路径区分：display-1 默认直线，display-3 默认 C 型
      const pathname = window.location && window.location.pathname ? window.location.pathname : '';
      if (typeof pathname === 'string' && pathname.includes('/display-3/')) {
        layoutMode = 'c-type';
      } else if (typeof pathname === 'string' && pathname.includes('/display-1/')) {
        layoutMode = 'linear';
      }
    }
    if (layoutMode === 'c-type') {
      root.classList.add('display-3');
      console.log('[DisplayWindowLogic] 布局: C型 (可由状态栏按钮切换为直线)');
    } else {
      root.classList.remove('display-3');
      console.log('[DisplayWindowLogic] 布局: 直线 (可由状态栏按钮切换为C型)');
    }
  } catch (e) {
    // 忽略解析失败
  }
  const isCTypeLayout = () => layoutMode === 'c-type';
  const setLayoutModeAndRedraw = (mode, redraw) => {
    if (mode !== 'linear' && mode !== 'c-type') return;
    layoutMode = mode;
    if (layoutMode === 'c-type') {
      root.classList.add('display-3');
    } else {
  ...
  // 状态栏在 renderDisp 定义之后创建，以便传入布局切换回调
  ...
  // 保存上一次顶栏“下一站/到达站/终点站”滚动渲染信息，用于缓存并避免频繁重建 DOM
  // 到达站模式下，当线路图切换到站名视图（asViewMode 由 0 -> 1）时需要强制刷新滚动动画
  let lastTopBarNextMarqueeInfo = {
    stationKey: null,
    cn: null,
    en: null,
    metaMode: null,
  ...
  const renderNormalScreen = (sts, meta) => {
        ...
        if (shouldScroll) {
          setTimeout(() => {
            if (span.offsetWidth > box.offsetWidth) {
              const w = span.offsetWidth; // 复制前宽度（用于估算速度）
              const gapHtml = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
              // 复制内容并添加间距，实现无缝滚动
    ...
    const currentStationKey = (
      (targetSt.id !== undefined && targetSt.id !== null) ? targetSt.id :
      (targetSt.idx !== undefined && targetSt.idx !== null) ? targetSt.idx :
      `${targetSt.name || ''}||${targetSt.en || ''}`
    );

    // 到达站模式下：当线路图切到站名视图（asViewMode 由 0 -> 1）时强制刷新滚动
    const forceRefreshByView =
      isArriving &&
      lastTopBarNextMarqueeInfo.asViewMode !== asViewMode &&
      asViewMode === 1;

    const nextMarqueeNeedsUpdate =
      lastTopBarNextMarqueeInfo.stationKey === null ||
      lastTopBarNextMarqueeInfo.stationKey !== currentStationKey ||
      lastTopBarNextMarqueeInfo.metaMode !== meta.mode ||
      lastTopBarNextMarqueeInfo.dirType !== meta.dirType ||
      lastTopBarNextMarqueeInfo.isArriving !== isArriving ||
      lastTopBarNextMarqueeInfo.isTerminal !== isTerminal ||
      forceRefreshByView;

    if (nextMarqueeNeedsUpdate) {
      nextStBox.innerHTML = '';
      const nWrapper = document.createElement('div');
      nWrapper.className = 'marquee-box';
      nWrapper.style.width = '100%';
      nWrapper.style.maxWidth = '100%';
      nWrapper.style.minWidth = '0';
      nWrapper.style.display = 'flex';
      nWrapper.style.flexDirection = 'column';
      nWrapper.style.alignItems = 'center';
      nWrapper.style.overflow = 'hidden';
      const nNameBox = document.createElement('div');
      nNameBox.className = 'marquee-box';
      nNameBox.style.width = '100%';
      ...
      nEnBox.style.width = '100%';
      nEnBox.style.maxWidth = '100%';
      nEnBox.style.minWidth = '0';
      ...
      // 更新缓存
      lastTopBarNextMarqueeInfo.stationKey = currentStationKey;
      lastTopBarNextMarqueeInfo.cn = targetSt.name || '';
      lastTopBarNextMarqueeInfo.en = targetSt.en || '';
      lastTopBarNextMarqueeInfo.metaMode = meta.mode;
      lastTopBarNextMarqueeInfo.dirType = meta.dirType;
      lastTopBarNextMarqueeInfo.isArriving = isArriving;
      lastTopBarNextMarqueeInfo.isTerminal = isTerminal;
  ...
  const renderArrivalScreen = (sts, meta) => {
    ...
    if (nextLbl && nextStBox) {
      ...
      // 到达站模式下顶栏滚动（中英文 marquee）做缓存：避免频繁重建导致动画被打断
      // 但当线路图切换到站名视图（asViewMode 由 0 -> 1）时强制刷新滚动
      const currentStationKey = (
        (st.id !== undefined && st.id !== null) ? st.id :
        (st.idx !== undefined && st.idx !== null) ? st.idx :
        `${st.name || ''}||${st.en || ''}`
      );
      const forceRefreshByView =
        lastTopBarNextMarqueeInfo.asViewMode !== asViewMode &&
        asViewMode === 1;
      const shouldReRenderMarquee =
        lastTopBarNextMarqueeInfo.stationKey === null ||
        lastTopBarNextMarqueeInfo.stationKey !== currentStationKey ||
        lastTopBarNextMarqueeInfo.metaMode !== meta.mode ||
        lastTopBarNextMarqueeInfo.dirType !== meta.dirType ||
        lastTopBarNextMarqueeInfo.isArriving !== true ||
        lastTopBarNextMarqueeInfo.isTerminal !== isTerminal ||
        forceRefreshByView;

      if (shouldReRenderMarquee) {
        // 更新站点名称显示
        nextStBox.innerHTML = '';
      const nWrapper = document.createElement('div');
      nWrapper.className = 'marquee-box';
      ...
      setTimeout(() => {
        ...
        // 更新缓存（仅当需要重建滚动时）
        lastTopBarNextMarqueeInfo.stationKey = currentStationKey;
        lastTopBarNextMarqueeInfo.cn = st.name || '';
        lastTopBarNextMarqueeInfo.en = st.en || '';
        lastTopBarNextMarqueeInfo.metaMode = meta.mode;
        lastTopBarNextMarqueeInfo.dirType = meta.dirType;
        lastTopBarNextMarqueeInfo.isArriving = true;
        lastTopBarNextMarqueeInfo.isTerminal = isTerminal;
    ...
    // 按「实际显示的站数」创建位置映射（与整条线总站数 total 无关）
    const displayCount = displaySts.length;
    if (displayCount <= effectiveMaxPositions) {
      const endPosition = effectiveMaxPositions - 1;
      positionMap = new Map();
      for (let i = 0; i < displayCount; i++) {
        const realIdx = sts.indexOf(displaySts[i]);
        if (i === 0) {
          positionMap.set(realIdx, 0);
        } else if (i === displayCount - 1) {
          positionMap.set(realIdx, endPosition);
        } else {
          const position = displayCount > 1 ? Math.round((i / (displayCount - 1)) * endPosition) : 0;
          positionMap.set(realIdx, position);
        }
      }
    }
    
    // 当实际显示站数 ≤ 7 时使用绝对定位 + arrivalNodeWidth
    if (displayCount <= effectiveMaxPositions && positionMap) {
      box.style.position = 'relative';
      box.style.width = '100%';
      box.style.height = '100%';
      const arrivalLeftPadding = 200;
      box.style.paddingLeft = arrivalLeftPadding + 'px';
      ...
      const trackWidth = endPositionIndex * arrivalNodeWidth + arrivalNodeWidth / 2;
      trackContainer.style.width = trackWidth + 'px';
      
      // 计算站点中心位置的辅助函数
      const getStationCenter = (stationIdx, posIndex) => {
        if (stationIdx === minIdx) {
          return arrivalNodeWidth / 2;
        }
        if (stationIdx === maxIdx) {
          return posIndex * arrivalNodeWidth + arrivalNodeWidth / 2;
        }
        return posIndex * arrivalNodeWidth;
      };
      ...
      // 计算可用宽度：从起点站中心到终点站中心
      const startCenter = arrivalNodeWidth / 2; // 起点站中心位置
      const endCenter = trackWidth - arrivalNodeWidth / 2; // 终点站中心位置
      ...
      displaySts.forEach((stNode, displayIdx) => {
        ...
        node.style.setProperty('width', arrivalNodeWidth + 'px', 'important');
        node.style.setProperty('min-width', arrivalNodeWidth + 'px', 'important');
        node.style.setProperty('max-width', arrivalNodeWidth + 'px', 'important');
        ...
        if (realIdx === rt.idx) {
          ...
          const highlightNameColor = (stNode && Array.isArray(stNode.xfer) && stNode.xfer.length > 0)
            ? '#c00'
            : getThemeColorByStationIndex(meta, realIdx, '#00b894');
          if (name) name.style.color = highlightNameColor;
          if (en) en.style.color = highlightNameColor;
        } else if (dot) {
...
// 到达站 7 个站：站名在圆点下方、倾斜与 C 型一致（-45°），最后一个字对准圆点；英文换行与 C 型一致
        const infoBtm = node.querySelector('.info-btm');
        if (infoBtm) {
          infoBtm.style.position = 'absolute';
          infoBtm.style.textAlign = 'end';
          infoBtm.style.transformOrigin = 'right center';
          infoBtm.style.transform = 'rotate(-45deg)';
          infoBtm.style.top = '60px';
      ...
      segments.forEach(segment => {
        ...
        // 计算两个站点的中心（与节点一致：全部用均匀间距 startCenter + i*spacing）
        center1 = startCenter + (st1DisplayIdx * spacing);
        center2 = startCenter + (st2DisplayIdx * spacing);
      ...
      const leftStationThemeColor = getThemeColorByStationIndex(meta, leftRealIdx, '#00b894');
      const rightStationThemeColor = getThemeColorByStationIndex(meta, rightRealIdx, '#00b894');
      const leftExtensionColor = forwardToRight ? '#ccc' : leftStationThemeColor;
      const rightExtensionColor = forwardToRight ? rightStationThemeColor : '#ccc';
      ...
      // 计算箭头位置（与节点一致：全部用均匀间距）
      const getArrowPosition = (i) => {
        if (i < 0 || i >= displayStsCount - 1) return 0;
        const currentCenter = startCenter + (i * spacing);
        const nextCenter = startCenter + ((i + 1) * spacing);
      ...
      box.querySelectorAll('.l-node').forEach((node) => {
        ...
        requestAnimationFrame(() => {
          requestAnimationFrame(() => {
            applyLeft();
            try { new ResizeObserver(() => { applyLeft(); }).observe(infoBtm); } catch (e) {}
          });
        });
      });
    } else {
      // 站点数量 > 7 时使用 flexbox，平均分布站间距
      mapPanel.style.paddingLeft = '20px';
      mapPanel.style.boxSizing = '';
      box.style.justifyContent = 'space-between';
      const flexNodeWidth = 160; // 每站固定宽度，保证站心距均匀
    displaySts.forEach((stNode) => {
      const realIdx = sts.indexOf(stNode);
      const node = mkNode(stNode, realIdx, 'linear', appData, rt);
      node.dataset.realIdx = String(realIdx);
      node.style.flex = `0 0 ${flexNodeWidth}px`;
      node.style.width = flexNodeWidth + 'px';
      node.style.minWidth = flexNodeWidth + 'px';
      node.style.maxWidth = flexNodeWidth + 'px';
      const dot = node.querySelector('.dot');
      // 判断是否已过站
      const isPassed = node.classList.contains('passed');
      const isStopAllowed = isStopAllowedAtIndex(realIdx, appData);
      ...
      // 到达站站名在圆点下方、倾斜与 C 型一致（-45°），最后一个字对准圆点
      const infoBtm = node.querySelector('.info-btm');
      if (infoBtm) {
        infoBtm.style.position = 'absolute';
        infoBtm.style.textAlign = 'end';
  ...
  const drawLinear = (c, sts, m) => {
    ...
    // 若为 C 型布局则增加 paddingBottom 以容纳更多内容
    c.style.paddingBottom = isCTypeLayout() ? '1000px' : '200px';
    ...
    // 若为 C 型布局，使用 C 型线路图（左侧开口、上绿下灰双轨、站点上下两排）
    if (isCTypeLayout()) {
      ...
      sts.forEach((st, idx) => {
        ...
        if (dot) {
          ...
          if (st.skip || isPassedNode || !isStopAllowedAtIndex(idx, appData)) {
            ...
            // 下一站：白底 + 线路主题色描边 + 放大呼吸动画
            const accent = getThemeColorByStationIndex(m, idx, '#00b894');
            dot.style.background = '#fff';
            dot.style.borderColor = accent;
            dot.style.boxShadow = 'none';
            dot.style.animation = 'dot-target-scale-pulse 1.1s ease-in-out infinite';
        ...
        if (idx === targetIdx) {
          ...
          if (infoBtm) {
            const nameEl = infoBtm.querySelector('.name');
            const enEl = infoBtm.querySelector('.en');
            const highlightNameColor = (st && Array.isArray(st.xfer) && st.xfer.length > 0)
              ? '#c00'
              : getThemeColorByStationIndex(m, idx, '#00b894');
            if (nameEl) nameEl.style.color = highlightNameColor;
            if (enEl) enEl.style.color = highlightNameColor;
          }
        }
        ...
        if (st && Array.isArray(st.xfer) && st.xfer.length > 0 && !st.skip && !node.classList.contains('passed') && isStopAllowedAtIndex(idx, appData)) {
          ...
          if (infoBtm) {
            const nameEl = infoBtm.querySelector('.name');
            const enEl = infoBtm.querySelector('.en');
            if (nameEl) nameEl.style.color = '#c00';
            if (enEl) enEl.style.color = '#c00';
          }
        }
      ...
      sts.forEach((st, i) => {
        ...
        if (i < rangeStart || i > rangeEnd) {
          ...
          if ((st.skip || !isStopAllowed) && dot) {
            ...
            // 下一站：白底 + 线路主题色描边 + 放大呼吸动画
            const accent = getThemeColorByStationIndex(m, i, '#00b894');
            dot.style.background = '#fff';
            dot.style.borderColor = accent;
            dot.style.boxShadow = 'none';
            dot.style.animation = 'dot-target-scale-pulse 1.1s ease-in-out infinite';
            const highlightNameColor = (st && Array.isArray(st.xfer) && st.xfer.length > 0)
              ? '#c00'
              : getThemeColorByStationIndex(m, i, '#00b894');
            if (name) name.style.color = highlightNameColor;
            if (en) en.style.color = highlightNameColor;
          } else if (dot) {
            // 如果已过站，使用灰色（保持passed类的样式）
  ...
  const drawRing = (c, sts, m) => {
    ...
    const RING_TRACK_LENGTH_MAX = 1670; // 环线高亮线路条长度上限
    const w = Math.min(RING_TRACK_LENGTH_MAX, Math.max(minTotalW, topCount * minSpacing));
    ...
    const RING_OFFSET_LEFT = 80; // 环线整体向左偏移
    const RING_OFFSET_TOP = 24; // 环线整体向上偏移
    const wrapper = document.createElement('div');
    wrapper.style.position = 'relative';
    wrapper.style.width = `${W}px`;
    wrapper.style.height = `${H}px`;
    wrapper.style.flexShrink = '0';
    wrapper.style.marginLeft = `-${RING_OFFSET_LEFT}px`;
    wrapper.style.marginTop = `-${RING_OFFSET_TOP}px`;
    ...
    if (rt.state === 1) {
      ...
      // 短交路：环线 getNextValidSt 会绕环，可能返回运营区外索引，与直线一致地限制在运营区内
      if (hasStartTerm && (targetIdx < minIdx || targetIdx > maxIdx)) {
        targetIdx = rt.idx;
      }
    ...
    for (let i = 0; i < sts.length; i++) {
      ...
      // 短交路：线段必须在运营区内才可高亮，运营区外（含左侧半环等）一律灰色
      const isInServiceRange = !hasStartTerm || (
      ...
      // 确定线段颜色
      let segmentColor = '#ccc'; // 默认灰色
      // 短交路：运营区外线段（含左侧半环）一律灰色，不高亮
      if (!isInServiceRange) {
        segmentColor = '#ccc';
      } else if (isLastToFirstSegment && isInHighlightRange) {
        // 首末站之间的线段在运营区内且在高亮范围内，使用主题色
    ...
    sts.forEach((st, i) => {
      ...
      // 环线站名倾斜：先按路径切线计算角度，在水平段（切线约 0°）改用固定倾角，与 C 型/参考图一致
      const distPrev = (dist - 8 + perimeter) % perimeter;
      const distNext = (dist + 8) % perimeter;
      const pPrev = measurePath.getPointAtLength(distPrev);
      const pNext = measurePath.getPointAtLength(distNext);
      let angle = Math.atan2(pNext.y - pPrev.y, pNext.x - pPrev.x) * 180 / Math.PI;
      const RING_TILT_DEG = 38; // 固定倾角（度），与 C 型风格一致
      const isNearHorizontal = Math.abs(angle) < 10 || Math.abs(Math.abs(angle) - 180) < 10;
      if (isNearHorizontal) {
        // 水平段（切线约 0° 或 180°）：上下半环都往左倾斜
        angle = -RING_TILT_DEG;
      }
      // 换乘/暂缓等标签移到环线内侧空白区域（两轨之间的空隙），不压在圆点上
      const xBox = label.querySelector('.x-box');
      if (xBox) {
        xBox.remove();
        node.insertBefore(xBox, node.firstChild); // 插入到节点最前（在 .dot 前）
        xBox.style.position = 'absolute';
        xBox.style.left = '50%';
        xBox.style.transform = 'translateX(-50%)';
        xBox.style.pointerEvents = 'none';
        xBox.style.zIndex = '12';
        xBox.style.writingMode = 'horizontal-tb';
        xBox.style.direction = 'ltr';
        // 与 C 型一致：横排布局，多标签一行显示；显式设 flex-direction 与 width 避免被父级压成竖排
        xBox.style.display = 'flex';
        xBox.style.flexDirection = 'row';
        xBox.style.flexWrap = 'wrap';
        ...
        // 上半环：圆点在上轨，内侧在下方 → 标签放在圆点下方
        // 下半环：圆点在下轨，内侧在上方 → 标签放在圆点上方
        if (isUpperHalf) {
          xBox.style.top = 'calc(50% + 20px)';
          xBox.style.transform = 'translate(-50%, 0)';
        } else {
          xBox.style.top = 'calc(50% - 20px)';
          xBox.style.transform = 'translate(-50%, -100%)';
        }
      }
      if (isUpperHalf) {
        // 上半环：第一个字靠近圆点（与 C 型一致），左边缘对齐圆点，左对齐文本
        label.style.top = '0';
        label.style.bottom = 'auto';
        label.style.left = '50%'; // 圆点在节点中心
        label.style.textAlign = 'left';
        label.style.transformOrigin = '0% 100%'; // 以标签左下为旋转中心，贴近圆点
        label.style.transform = `translate(0, calc(-100% - 10px)) rotate(${angle}deg)`;
      } else {
        // 下半环：最后一个字靠近圆点（与 C 型一致），右边缘对齐圆点，右对齐文本；缩小与圆点间距
        label.style.top = '16px';
        label.style.bottom = 'auto';
        label.style.left = '50%'; // 圆点在节点中心
        label.style.textAlign = 'right';
        label.style.transformOrigin = '100% 0%'; // 以标签右上为旋转中心，贴近圆点
      ...
      if ((st.skip || !isStopAllowed) && dot) {
        ...
        if (label) {
          const highlightNameColor = (st && Array.isArray(st.xfer) && st.xfer.length > 0)
            ? '#c00'
            : getThemeColorByStationIndex(m, i, '#00b894');
          label.style.color = highlightNameColor;
          const enEl = label.querySelector('.en');
          if (enEl) enEl.style.color = highlightNameColor;
        }
      ...
      const isOutsideRange = hasStartTerm && (
        (minIdx <= maxIdx && (i < minIdx || i > maxIdx)) ||
        (minIdx > maxIdx && i > maxIdx && i < minIdx)
      );
      if (isOutsideRange && dot) {
        dot.style.background = '#fff';
        dot.style.borderColor = '#ccc';
        dot.style.width = '24px';
        dot.style.height = '24px';
      ...
      // 短交路：仅运营区内线段显示箭头，运营区外不显示（与直线模式一致）
      const isInServiceRange = !hasStartTerm || (
        (minIdx <= maxIdx && i >= minIdx && i < maxIdx) ||
        (minIdx > maxIdx && (i >= minIdx || i < maxIdx))
      );
  ...
  const handleBroadcastMessage = (event) => {
    ...
    if (data.t === 'SYNC') {
      ...
      rt = data.r || rt;
      // 主应用下发的线路图布局（显示端设置中的「线路图布局」）
      const pathname = typeof window !== 'undefined' && window.location && window.location.pathname ? window.location.pathname : '';
      const preferredLayout = pathname.includes('/display-3/')
        ? (data.settings && data.settings.display && data.settings.display.display3LayoutMode)
        : (data.settings && data.settings.display && data.settings.display.display1LayoutMode);
      if (preferredLayout === 'linear' || preferredLayout === 'c-type') {
