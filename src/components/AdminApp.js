...
export default {
  ...
  setup() {
    ...
    // 站点管理列表：站名旁“音频缺失”红色三角形提示
    const stationAudioBrokenByIdx = ref({})
    const stationAudioBrokenScanTimer = ref(null)
    let stationAudioBrokenScanToken = 0

    const scanStationAudioBroken = async () => {
        if (stationAudioBrokenScanTimer.value) {
            clearTimeout(stationAudioBrokenScanTimer.value)
            stationAudioBrokenScanTimer.value = null
        }
        stationAudioBrokenScanTimer.value = setTimeout(async () => {
            ...
            try {
                ...
                for (let i = 0; i < stations.length; i++) {
                    ...
                    if (token !== stationAudioBrokenScanToken) return
                }

                if (token === stationAudioBrokenScanToken) stationAudioBrokenByIdx.value = next
            } catch (e) {
                // keep silent; UI should not crash
            }
        }, 160)
    }
    ...
    // 当线路/站点数据变化时，刷新“音频缺失”提示
    watch(
        () => state.currentFilePath,
        () => scanStationAudioBroken(),
        { immediate: true }
    )
    watch(
        () => state.appData?.stations,
        () => scanStationAudioBroken(),
        { deep: true }
    )

    onMounted(() => scanStationAudioBroken())
