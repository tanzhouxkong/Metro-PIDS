...
## 更新日志
#### 新功能
- **贯通线路（Through Operation）支持**：
  - 支持将多条线路合并为一条贯通线路，实现跨线路的站点连接与显示
  - 支持选择多个线路段进行贯通，每条线路段可指定线路名称和贯通站点
  - 自动检测相邻线路段之间的共同站点作为贯通站点，简化配置流程
  - 合并后的贯通线路保留各线路段的主题颜色，在显示时按颜色范围区分不同线路段
  - 自动创建贯通线路并命名为"线路1 - 线路2 (贯通)"格式，创建后自动切换到新线路
  - 支持清除贯通设置，可重新配置贯通线路
