...
- 快捷键控制：支持配置"下一步/上一站/到达/发车"等快捷键，并在显示端按键时转发控制端执行。第三方显示器通过 SDK 的 `installKeyboardHandler()` 可以完全支持用户自定义快捷键。
...
- 支持为每个站点配置到站/发车音频，支持上行与下行分离。
- 支持通用音频（`commonAudio`）快捷触发（`1~0`、`F1~F12`）。
- 在站点管理页可通过右键菜单对当前站点执行：**播放**。
- 录屏时可按设置将车站音频事件写入时间线并在导出时混音。

建议使用流程：

1. 打开站点编辑器，为站点添加音频文件（会复制到线路目录 `audio/`）。
2. 在音频条目中设置触发模式（到站/发车/运营模式）。
...
第三方显示器现在**完全支持用户自定义快捷键**，与显示器1的行为完全一致。

#### 推荐方式：使用 SDK 的键盘事件处理器

**这是最推荐的方式**，第三方显示器只需要安装键盘事件处理器，主程序会根据用户配置的快捷键自动匹配并执行对应操作：
...
如果不想使用键盘事件处理器，也可以：
- **手动发送语义化命令**：发送 `{ t: 'CMD_KEY', command: 'next' }`，主程序直接执行对应操作（不依赖快捷键配置）
- **手动发送按键信息**：发送 `{ t: 'CMD_KEY', code: 'ArrowRight', key: 'ArrowRight' }`，主程序检查是否与用户配置的快捷键匹配
