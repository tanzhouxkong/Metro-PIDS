...
第三方显示器可以通过 BroadcastChannel 发送控制命令到主程序：
...
function sendCommand(command) {
  ...
  else if (command === 'next') keyCode = 'ArrowRight';
  else if (command === 'arrive') keyCode = 'Enter';
  else if (command === 'depart') keyCode = 'Space';
  ...
  if (bc) {
    try {
      ...
      console.log('✅ 控制命令已通过广播发送:', command, '->', keyCode);
      return true;
    } catch (e) {
...
// 使用示例
sendCommand('next');    // 下一站
sendCommand('prev');    // 上一站
sendCommand('arrive');  // 到达
sendCommand('depart');  // 发车
```
...
请参考 `examples/third-party-display-template.html` 文件，其中包含了一个完整的第三方显示器实现示例。
